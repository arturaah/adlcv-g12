...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_logits = queries @ keys.permute(0, 2, 1)  # multiply queries and keys
        attention_logits = attention_logits * self.scale
        attention = torch.nn.Softmax()(attention_logits)  # softmax on attention
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_seq_len=512):
        ...
        div_term = torch.arange(0.0, embed_dim, 2)
...
class TransformerClassifier(nn.Module):
    def __init__(
        ...
        self.token_embedding = nn.Embedding(
            embedding_dim=embed_dim, num_embeddings=num_tokens
        )
