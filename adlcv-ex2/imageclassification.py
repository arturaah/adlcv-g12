...
def select_two_classes_from_cifar10(dataset, classes):
    idx = (np.array(dataset.targets) == classes[0]) | (
        np.array(dataset.targets) == classes[1]
    )
    ...
    for e in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            for image, label in test_iter:
                if torch.cuda.is_available():
                    image, label = image.to("cuda"), label.to("cuda")
                out, maps = model(image)
                fig, ax = plt.subplots(5, 4, figsize=(4, 2))
                for i in range(16):
                    ax[0, i].imshow(image[i].permute(1, 2, 0).cpu().numpy())
                    ax[0, i].axis("off")
                    ax[1, i].imshow(maps[i].cpu().numpy())
                    ax[1, i].axis("off")
            ...
            print(
                f"-- train loss {train_loss:.3f} -- validation accuracy {acc:.3f} -- validation loss: {val_loss:.3f}"
            )
