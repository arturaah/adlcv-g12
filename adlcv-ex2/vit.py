...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        self.attention_maps = self.attention.attention_maps
        # plt.imshow(spatial_attention.cpu().detach().numpy())
...
class ViT(nn.Module):
    def __init__(
        ...
        self.to_patch_embedding = nn.Sequential(
            Rearrange(
                "b c (nh ph) (nw pw) -> b (nh nw) (c ph pw)", ph=patch_h, pw=patch_w
            ),
    ...
    def forward(self, img):
        ...
        attention_maps = [blk.attention_maps for blk in self.transformer_blocks]
