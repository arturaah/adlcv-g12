...
def positional_encoding_2d(nph, npw, dim, temperature=10000, dtype=torch.float32):
    """nph=number of patches in height
        npw= number of patches in width
        dim=embedding dimensions
        temperature=scaling factor for the embeddings
        dtype= data type of the output tensor"""
    y, x = torch.meshgrid(torch.arange(nph), torch.arange(npw), indexing="ij") # creates a 2D coordinate system. Takes two 1D arrays and returns two 2D arrays. 'ij': ensures matrix indexing convention (instead of Cartesian)
    assert (dim % 4) == 0, "feature dimension must be multiple of 4 for sincos emb"#We need 4 components [x_sin, x_cos, y_sin, y_cos]. Each component needs equal space in the final embedding
    
    omega = torch.arange(dim // 4) / (dim // 4 - 1)
    omega = 1.0 / (temperature ** omega)
    """Creates frequencies for the sinusoidal encoding
        dim // 4: quarter of dimension (space for each sin/cos component)
        temperature ** omega: creates exponentially decreasing frequencies
        Higher frequencies capture fine-grained positions
        Lower frequencies capture coarse positions"""
        # Coordinate Encoding
    y = y.flatten()[:, None] * omega[None, :]#[:, None] adds a dimension for broadcasting. Multiplies each coordinate with all frequencies. y.flatten() → [0, 0, 0, 1, 1, 1] (1D array)[:, None] → [[0], [0], [0], [1], [1], [1]] (2D array, shape: [6, 1])
    x = x.flatten()[:, None] * omega[None, :]
    #Tensors shape before concatenation
    """ x.sin(): [num_patches, dim//4]
        x.cos(): [num_patches, dim//4]
        y.sin(): [num_patches, dim//4]
        y.cos(): [num_patches, dim//4]"""
    pe = torch.cat((x.sin(), x.cos(), y.sin(), y.cos()), dim=1)# Concatenation of tensors along specified dimension(dim=1 --> feature dimension)dim=0 would stack vertically (increase number of patches)dim=1 stacks horizontally (combines features)
    return pe.type(dtype)# converts the positional encoding into a specified data type
#The final position(i,j) encoding will look like:
"""pe[i,j] = [sin(x_i*ω_1), ..., sin(x_i*ω_d/4),
           cos(x_i*ω_1), ..., cos(x_i*ω_d/4),
           sin(y_j*ω_1), ..., sin(y_j*ω_d/4),
           cos(y_j*ω_1), ..., cos(y_j*ω_d/4)]"""
