...
classifier.eval()
classifier.load_state_dict(torch.load('ex6/weights/classifier/model.pth', map_location=device))

unet = UNet(device=device)
unet.eval()
unet.to(device)
unet.load_state_dict(torch.load('ex6/weights/DDPM/model.pth', map_location=device))
...
show(imgs, fig_titles=CLASS_LABELS, title='Classifier Guidance', save_path='ex6/assets/cg_samples.png')
