...
def create_result_folders(experiment_name):
    os.makedirs("ex6/weights", exist_ok=True)
    os.makedirs("ex6/results", exist_ok=True)
    os.makedirs(os.path.join("ex6/weights", experiment_name), exist_ok=True)
    os.makedirs(os.path.join("ex6/results", experiment_name), exist_ok=True)
...
def train(T=500, cfg=True, img_size=16, input_channels=3, channels=32, 
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if epoch_loss <= min_train_loss:
            torch.save(model.state_dict(), os.path.join("ex6/weights", experiment_name, f"model.pth"))
        ...
        save_images(images=sampled_images, path=os.path.join("ex6/results", experiment_name, f"{epoch}.jpg"),
